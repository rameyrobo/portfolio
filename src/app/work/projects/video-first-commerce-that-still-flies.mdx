...
- **Lightweight UI:** Tailwind utilities with **reusable components** and minimal runtime overhead; Turbopack for faster local iterations.

## Engineering highlights

- **Deep linking** for product modals so each PDP has a shareable URL.
...
- **Password‑protected promo** instance (hush‑hush) for pre‑launch screening.
...
- Migrated from `order.completed` to **`cart.confirmed`** events to ensure the upload UI only appears after a successful purchase.
